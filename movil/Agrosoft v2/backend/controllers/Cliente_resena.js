    ...
    try {
        ...
        const reviews = await sequelize.query(
            ...
            SELECT
                ...
                cr.calificacion,
                cr.fecha_creacion,
                cr.estado_comentario,
                p.id_producto,
                p.nombre_producto
            FROM comentario_resena cr
            JOIN producto p ON cr.id_producto = p.id_producto
            WHERE cr.id_usuario = ? AND cr.estado_comentario != 'Eliminado'
            ORDER BY cr.fecha_creacion DESC
            `,
        ...
        res.status(500).json({ success: false, message: "Error al obtener reseñas por usuario" });
    }
};
